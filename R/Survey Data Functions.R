...
# These functions are probably inefficiently coded, but they seem to work. They in part trace my slow
# learning / development in the R language. Feel free to suggest improvements.

# Most of the time when we work with surveys at Conestoga we end up creating a weighting variable, so most 
# of the functions here that summarize data [eg. ftw()] have a weight required. If you don't have a weight, 
# one way around it is just to make a column with a value of 1 in it for all cases.
...
# For battery questions, we use battery_ftw() and frq_g_battery(), which is our stacked bar.
...
# Currently requires a weight variable.
# --------------------------------------------------------------------------------------------
ftw = function(df, Question_var_in_Quotes, Weight_var_in_Quotes = "weight", count_round = TRUE) {
  df = df %>% 
    select(Question_var_in_Quotes, Weight_var_in_Quotes) %>% 
    rename(resp = Question_var_in_Quotes, w = Weight_var_in_Quotes) %>% 
    drop_na %>%
    group_by(resp) %>% 
...
#   weight = Specific weighting variable, in quotes. Required.
...
# --------------------------------------------------------------------------------------------
ct = function(dataset, Row_Demo_quoted, Column_Question_quoted, weight = "weight", pct = FALSE, decimals = NULL, count = FALSE, include_N = TRUE) {
  Data = dataset %>% 
    { if (is.null(weight)) select(., Column_Question_quoted, Row_Demo_quoted) else select(., Column_Question_quoted, Row_Demo_quoted, weight) } %>%
    rename(Response = 1, Field = 2) %>% 
    { if (is.null(weight)) mutate(., weight = 1) else rename(., weight = 3) }
  CrossTab = Data %>% 
...
# ------------------------------------------------------------------------------------------------------------
ctable = function(df, Col1Name = "", Col1Width = "20%", OtherColWidths = "10%", chi2 = TRUE, freq = FALSE, decimals = NULL) {
  # Assign Col1Name, if not default to supplied variable title
  if (Col1Name != "") { names(df)[1] = Col1Name } 
  if (chi2 == TRUE) {
    ...
    if (nrow(data) > 1) {
      ...
      chi2 = FALSE
      message("Dataframe supplied only contains one effective row - disabling chi-square test results reporting.")
    }
  }
  ...
  CrossTab = datatable(df, class = 'row-border', rownames = FALSE, options = list(scrollX = FALSE, paging = FALSE, searching = FALSE, dom = 't', ordering = F, autoWidth = TRUE, 
...
# Mark Kane's MultiCoding Function, for multi-response questions from Qualtrics (comma-separated options).
# --------------------------------------------------------------------------------------------
...
# Relies on Mark's MultiCoding function. Currently requires a weight variable.
# Default mode (NULL) takes the default Qualtrics output of a comma-separated-mulit-response "Q" and calls MultiCode to get an array of dummies that it then summarizes, using the weights supplied.
# Modified mode (!NULL) ignores weight calculations - simply creates a frequency table based on supplied values in the df - they need to be numbers that can be summed. N = number of rows.
# Sometimes for graphs you need to get an N out of here - as in, how many people answered these multi-select questions in any fashion. For this, use the return_N feature (set to TRUE).
# --------------------------------------------------------------------------------------------
mc_ftw = function(df, Q = NULL, weight = "weight", mode = NULL, return_N = FALSE) {
  if (is.null(mode)) {
    mc = MultiCoding(df, Q)
    df = select(df, all_of(weight)) %>%
      rename(weight = 1) %>% 
      bind_cols(., mc) %>% 
...
# --------------------------------------------------------------------------------------------
battery_ftw = function(df, Q_prefix_quoted, weight = "weight", freq = FALSE, round_freq = TRUE, skip_Questions = FALSE) {
  temp = select(df, weight, contains(Q_prefix_quoted)) %>% 
    rename(weight = 1) %>% 
...
# --------------------------------------------------------------------------------------------
frq_g_simple = function(df, Title_in_Quotes = "", Title_wrap_length = 55, Title_font_size = 16, Subtitle_font_size = 14, Value_font_size = 6, Cat_wrap_length = 25, Cat_font_size = 20, Custom_N = NULL, Cat_pcts = TRUE) {
  df = df %>%  
  ...
  df %>% 
    ...
    xlim(rev(df$resp)) + 
    geom_text(aes(label = {if (Cat_pcts == TRUE) pct_format(pct) else pct}), size = Value_font_size, position = position_stack(vjust = 0.9)) +
    coord_flip() +
...
# --------------------------------------------------------------------------------------------
frq_g_battery = function(df, Title_in_Quotes = NULL, Title_wrap_length = 55, Title_font_size = 16, Subtitle_font_size = 14, Cat_font_size = 12, Cat_wrap_length = 34, YRightMargin = 0, Fcolour = "Blues", Label_font_size = 5, N_mode = "t", Legend_Rows = 2, Legend_Padding = 100, subtitle = NULL, colours = NULL) { 
  ...
  Table %>% 
    ...
    geom_text(aes(label = ifelse(pct>=0.045,paste0(round(pct*100,0),"%"),"")), size = Label_font_size, position = position_stack(vjust = 0.5)) +
    coord_flip() +
